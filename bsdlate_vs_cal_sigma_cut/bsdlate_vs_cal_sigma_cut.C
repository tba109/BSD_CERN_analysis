#define bsdlate_vs_cal_sigma_cut_cxx
#include "bsdlate_vs_cal_sigma_cut.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
#include <TGraph.h>
...
void bsdlate_vs_cal_sigma_cut::Loop()
{
  Int_t index = 0;
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) 
      ...
      if( ptype == 1 && (penergy >=75 || penergy <= 175) )
	{
	  ...
	  aE[index][1][NE[index]] = myBSDLatePE;
	  NE[index]++;
	  ...
	  if( (CALSum < (means[index] - 2*sigmas[index])) ||
	      (CALSum > (means[index] + 2*sigmas[index])) )
	    {
	      ...
	      aE_2sigma[index][1][NE_2sigma[index]] = myBSDLatePE;
	      NE_2sigma[index]++;
	    }
	  ...
	  if( (CALSum < (means[index] - 3*sigmas[index])) ||
	      (CALSum > (means[index] + 3*sigmas[index])) )
	    {
	      ...
	      aE_3sigma[index][1][NE_3sigma[index]] = myBSDLatePE;
	      NE_3sigma[index]++;
	    }
	}
    }
  ...
  grE75->GetYaxis()->SetTitle("75 GeV myBSDLatePE (PE)");
  ...
  grE100->GetYaxis()->SetTitle("100 GeV myBSDLatePE (PE)");
  ...
  grE125->GetYaxis()->SetTitle("125 GeV myBSDLatePE (PE)");
  ...
  grE150->GetYaxis()->SetTitle("150 GeV myBSDLatePE (PE)");
  ...
  grE175->GetYaxis()->SetTitle("175 GeV myBSDLatePE (PE)");
