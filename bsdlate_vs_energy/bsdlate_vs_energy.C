#define bsdlate_vs_energy_cxx
#include "bsdlate_vs_energy.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
#include <iostream>
...
void bsdlate_vs_energy::Loop1()
{
    if (fChain == 0) return;

  TH1F * hPi350 = new TH1F("hPi350","",200,-100,8000);
  TH1F * hPi300 = new TH1F("hPi300","",200,-100,8000);
  TH1F * hPi250 = new TH1F("hPi250","",200,-100,8000);

  TH1F * hE175 = new TH1F("hE175","",200,-100,3000);
  TH1F * hE150 = new TH1F("hE150","",200,-100,3000);
  TH1F * hE125 = new TH1F("hE125","",200,-100,3000);
  TH1F * hE100 = new TH1F("hE100","",200,-100,3000);
  TH1F * hE75 = new TH1F("hE75","",200,-100,3000);
  TH1F * hE50 = new TH1F("hE50","",200,-100,3000);
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) 
	...
	myBSDLatePE += myfCHA[i]*0.25E-12/(gainA[i]*1.E6*1.6E-19);
      ...
      // 350 GeV pions
      if( ptype == 2 && penergy == 350) hPi350->Fill(myBSDLatePE);
      
      // 300 GeV pions
      if( ptype == 2 && penergy == 300) hPi300->Fill(myBSDLatePE);

      // 250 GeV pions
      if( ptype == 2 && penergy == 250) hPi250->Fill(myBSDLatePE);
      
      // 175 GeV electrons
      if( ptype == 1 && penergy == 175) hE175->Fill(myBSDLatePE);

      // 150 GeV electrons
      if( ptype == 1 && penergy == 150) hE150->Fill(myBSDLatePE);
      
      // 125 GeV electrons
      if( ptype == 1 && penergy == 125) hE125->Fill(myBSDLatePE);

      // 100 GeV electrons
      if( ptype == 1 && penergy == 100) hE100->Fill(myBSDLatePE);
      
      // 75 GeV electrons
      if( ptype == 1 && penergy == 75) hE75->Fill(myBSDLatePE);
      
      // 50 GeV electrons
      if( ptype == 1 && penergy == 50) hE50->Fill(myBSDLatePE);
    }
  ...
  TH2F * haxesPi = new TH2F("haxesPi","",100,-100,8000,100,0,600);
  haxesPi->SetStats(kFALSE);
  haxesPi->GetXaxis()->SetTitle("myBSDLatePE for Pion Energy Scan");
  haxesPi->GetXaxis()->CenterTitle();
  haxesPi->Draw();
  ...
  TH2F * haxesE = new TH2F("haxesE","",100,-100,3000,100,0,300);
  haxesE->GetXaxis()->SetTitle("myBSDLatePE for Electron Energy Scan");
  haxesE->GetXaxis()->CenterTitle();
...
// This one scales for equal areas
void bsdlate_vs_energy::Loop2()
{
  if (fChain == 0) return;
  
  TH1F * hPi350 = new TH1F("hPi350","",50,-100,8000);
  TH1F * hPi300 = new TH1F("hPi300","",50,-100,8000);
  TH1F * hPi250 = new TH1F("hPi250","",50,-100,8000);
  ...
  TH2F * haxesPi = new TH2F("haxesPi","",100,-100,8000,100,0,0.018);
  haxesPi->SetStats(kFALSE);
  haxesPi->GetXaxis()->SetTitle("myBSDLatePE for Pion Energy Scan");
  haxesPi->GetXaxis()->CenterTitle();
  haxesPi->Draw();
  ...
  TH2F * haxesE = new TH2F("haxesE","",100,-100,8000,100,0,0.1);
  haxesE->GetXaxis()->SetTitle("myBSDLatePE for Electron Energy Scan");
  haxesE->GetXaxis()->CenterTitle();
  haxesE->SetStats(kFALSE);
  haxesE->Draw();
