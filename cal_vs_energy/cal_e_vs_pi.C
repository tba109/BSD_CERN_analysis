#define cal_e_vs_pi_cxx
#include "cal_e_vs_pi.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>

void cal_e_vs_pi::Loop()
{
  if (fChain == 0) return;
  
  TH1F * hPi = new TH1F("hPi","",200,-1000,30000);
  TH1F * hE = new TH1F("hE","",200,-1000,30000);
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) 
	...
	myBSDLatePE += myfCHA[i]*0.25E-12/(gainA[i]*1.E6*1.6E-19);
      ...
      // 250-350 GeV pions
      if( ptype == 2 && (penergy <=  350 || penergy >= 250) ) hPi->Fill(CALSum);
      
      // 75-175 GeV electrons
      if( ptype == 1 && (penergy <= 175 || penergy >= 75) ) hE->Fill(CALSum);
  ...
  TH2F * haxes = new TH2F("haxes","",100,-1000,30000,100,0,0.035);
  haxes->SetStats(kFALSE);
  haxes->GetXaxis()->SetTitle("CALSum for 75-175 GeV Electrons (Red) and 250-350 GeV Pions (Blue) (detector units)");
